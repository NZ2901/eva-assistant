...
describe('BrainService', () => {
  it('executes an action, persists its confirmation and skips the LLM', async () => {
    const memoryService = {
      ensureConversation: jest.fn(),
      saveMessage: jest.fn(),
    };
    const action = { name: 'open_website', website: 'youtube' } as const;
    const actionExecutor = {
      execute: jest.fn().mockResolvedValue({ response: 'Abri o YouTube.' }),
    };
    const conversationService = { chat: jest.fn(), stream: jest.fn() };
    const command = {
    ...
    const service = new BrainService(
      ...
      { saveMemory: jest.fn() } as never,
      { extract: jest.fn().mockResolvedValue({ memories: [] }) } as never,
      conversationService as never,
      command as never,
      command as never,
      command as never,
      { resolve: jest.fn().mockReturnValue(action) } as never,
